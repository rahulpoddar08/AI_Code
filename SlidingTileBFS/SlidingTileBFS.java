...
class SlidingTileBFS{
	...
	public static void main(String args[]){
		...
		SlidingTileBFS e = new SlidingTileBFS(); 		
		node.setState(str);
		node.setParent(null);
		...
		while(e.q.peek()!=null){
			...
			//node.setState(top);
			//node.setParent(null);
			e.leftjump1(e.q.peek());
			e.rightjump1(e.q.peek());
			e.left(e.q.peek());					// Move the blank space up and add new state to queue
			e.right(e.q.peek());					// Move the blank space down 
			e.leftjump2(e.q.peek());					// Move left
			e.rightjump2(e.q.remove());				// Move right and remove the current node from Queue
	...
	public void printSolution(){
		...
		File file = new File("solution_slidingtile_bfs.txt");
		try{
